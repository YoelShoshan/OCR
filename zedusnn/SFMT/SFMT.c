...
int fill_array32(uint32_t *array, int size) {   
     ...
     */
		...
		#else
		int res = posix_memalign(((void**)&ptr,16, sizeof(uint32_t) * extended_size);
		assert(0==res);
		#endif
    #else	
		#if defined(_MSC_VER)
		...
		#else	
		int res = posix_memalign((void**)&ptr, __alignof(uint32_t),sizeof(uint32_t) * extended_size);
		assert(0==res);
		#endif
    #endif
	...
	gen_rand_array((w128_t*)ptr, extended_size / 4);
...
int fill_array64(uint64_t *array, int size) {
     ...
     */
		...
		#else
			int res = posix_memalign((void**)&ptr,16, sizeof(uint64_t) * extended_size);
			assert(0==res);
		#endif
    #else
		#if defined(_MSC_VER)
		...
		#else
			int res = posix_memalign((void**)&ptr,__alignof(uint64_t), sizeof(uint64_t) * extended_size);
			assert(0==res);
		#endif
    #endif
    ...
    if (ptr == NULL)
      ...
      gen_rand_array((w128_t *)ptr, extended_size / 2);
...
void init_gen_rand(uint32_t seed) {
	//printf("init_gen_rand::1\n");
    int i;

    psfmt32[idxof(0)] = seed;
    printf("init_gen_rand::2\n");
    for (i = 1; i < N32; i++) {
	psfmt32[idxof(i)] = 1812433253UL * (psfmt32[idxof(i - 1)] 
					    ^ (psfmt32[idxof(i - 1)] >> 30))
	    + i;
    }
    printf("init_gen_rand::3\n");
    idx = N32;
    printf("init_gen_rand::4\n");
    period_certification();
    printf("init_gen_rand::5\n");
    initialized = 1;
}
